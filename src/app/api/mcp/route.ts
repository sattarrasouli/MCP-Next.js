...
const handler = createMcpHandler(
  (server) => {
    ...
    server.tool(
      "emoji_reaction",
      "Responds with a random emoji reaction",
      {},
      async () => {
        const emojis = ["😃", "👍", "🎉", "🥳", "🚀", "🙌", "😎"];
    ...
    server.tool(
      ...
      async ({ count }) => {
        ...
        return {
          content: [
            { type: "text", text: `🎨 Your color palette: ${palette.join(", ")}` }
          ],
        };
      }
    );
    ...
    server.tool(
      ...
      async ({ sign }) => {
        ...
        const message = horoscopes[lower] || "Please provide a valid zodiac sign (e.g., Aries, Taurus, Gemini, ...)";
        return {
